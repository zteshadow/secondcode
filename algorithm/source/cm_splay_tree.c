...
//一字旋转，天平旋转
static SearchTree cm_balance_rotation_with_left(SearchTree t);
static SearchTree cm_balance_rotation_with_right(SearchTree t);
...
static SearchTree cm_balance_rotation_with_left(SearchTree grand)
{
    SearchTree father = grand->left;
    SearchTree son = father->left;
...
static SearchTree cm_balance_rotation_with_right(SearchTree grand)
{
    SearchTree father = grand->right;
    SearchTree son = father->right;
...
SearchTree cm_binary_tree_delete(SearchTree tree, S32 value)
{
    ...
    if (tree != 0)
    {
        ...
        else
        {
            ...
            {
                Position p = cm_binary_tree_find_min(tree->right);
                if (p != 0)
                {
                    tree->value = p->value;
...
Position cm_binary_tree_find(SearchTree tree, S32 value)
{
    ...
    if (tree != 0)
    {
        if (value < tree->value)
        {
            tree->left = cm_binary_tree_find(tree->left, value);
            tree = cm_single_rotation_with_left(tree);
        }
        else if (value > tree->value)
        {
            tree->right = cm_binary_tree_find(tree->right, value);
...
Position cm_binary_tree_find(SearchTree tree, S32 value)
{
    ...
    if (tree != 0)
    {
        ...
        {
            t = tree->left;
            if (value < t->value)          //zig-zig
            {
                t->left = cm_binary_tree_find(t->left, value);
                tree = cm_balance_rotation_with_left(tree);
            }
            else if (value > t->value)     //zig-zag
            {
                t->right = cm_binary_tree_find(t->right, value);
        ...
        else if (value > tree->value)
        {
            ...
            {
                t->left = cm_binary_tree_find(t->left, value);
                tree = cm_balance_rotation_with_right(tree);
            }
            else
            {
                tree = cm_single_rotation_with_right(tree);
