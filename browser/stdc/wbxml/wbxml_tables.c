...
const WBXMLAttrEntry sv_wml10_attr_table[] = {
    ...
    { "height",          {0},                                0x00, 0x13 },
    { "hspace",          {0},                                0x00, 0x14 },
    { "idefault",        {0},                                0x00, 0x15 }, /* Deprecated */
    { "ikey",            {0},                                0x00, 0x16 }, /* Deprecated */
    { "key",             {0},                                0x00, 0x17 }, /* Deprecated */
    { "label",           {0},                                0x00, 0x18 },
    { "localsrc",        {0},                                0x00, 0x19 },
    { "maxlength",       {0},                                0x00, 0x1a },
    ...
    { "onclick",         {0},                                0x00, 0x24 }, /* Deprecated */
    { "onenterbackward", {0},                                0x00, 0x25 },
    { "onenterforward",  {0},                                0x00, 0x26 },
    { "ontimer",         {0},                                0x00, 0x27 },
    { "optional",        "false",                             0x00, 0x28 },
    { "optional",        "true",                              0x00, 0x29 },
    { "path",            {0},                                0x00, 0x2a },
    { "postdata",        {0},                                0x00, 0x2b }, /* Deprecated */
    ...
    { "type",            "text",                              0x00, 0x48 },
    { "type",            "vnd.",                              0x00, 0x49 },
    { "url",             {0},                                0x00, 0x4a }, /* Deprecated */
    { "url",             "http://",                           0x00, 0x4b }, /* Deprecated */
    { "url",             "https://",                          0x00, 0x4c }, /* Deprecated */
    { "user-agent",      {0},                                0x00, 0x4d }, /* Deprecated */
    { "value",           {0},                                0x00, 0x4e },
    { "vspace",          {0},                                0x00, 0x4f },
    { "width",           {0},                                0x00, 0x50 },
    { "xml:lang",        {0},                                0x00, 0x51 },
    { {0},              {0},                                0x00, 0x00 }
};
...
const WBXMLAttrEntry sv_wml11_attr_table[] = {
    ...
    { "align",           "top",                               0x00, 0x0b },
    { "alt",             {0},                                0x00, 0x0c },
    { "class",           {0},                                0x00, 0x54 }, /* WML 1.1 */
    { "columns",         {0},                                0x00, 0x53 }, /* WML 1.1 */
    { "content",         {0},                                0x00, 0x0d },
...
const WBXMLAttrEntry sv_channel12_attr_table[] = {
    ...
    { "channelid",          {0},       0x00, 0x15 },
    { "useraccessible",     {0},       0x00, 0x16 },
    { {0},                 {0},       0x00, 0x00 }
};
...
const WBXMLAttrEntry sv_prov10_attr_table[] = {
    ...
    { "name",        {0},                  0x01, 0x05 }, /* OMA */
    { "value",       {0},                  0x01, 0x06 }, /* OMA */
...
const WBXMLAttrEntry sv_drmrel10_attr_table[] = {
    { "xmlns:o-ex",     {0},       0x00, 0x05 },
    { "xmlns:o-dd",     {0},       0x00, 0x06 },
    { "xmlns:ds",       {0},       0x00, 0x07 },
    { {0},             {0},       0x00, 0x00 }
};
...
#if defined( WBXML_SUPPORT_WML )
#ifdef WBXML_TABLES_SEPARATE_WML_VERSIONS    
    { WBXML_LANG_WML10, NULL, NULL, NULL, NULL, NULL, NULL },
    { WBXML_LANG_WML10, NULL, NULL, NULL, NULL, NULL, NULL },
    { WBXML_LANG_WML10, NULL, NULL, NULL, NULL, NULL, NULL },
...
{
    static WB_LONG s_bd_sv_table_init = 0;

    if (s_bd_sv_table_init == 0)
    {
        WBXMLLangEntry *p = sv_table_entry;

#if defined( WBXML_SUPPORT_WML )
#ifdef WBXML_TABLES_SEPARATE_WML_VERSIONS
        p->langID = WBXML_LANG_WML10;
        p->publicID = &sv_wml10_public_id;
        p->tagTable = sv_wml10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wml10_attr_table;
        p->attrValueTable = sv_wml10_attr_value_table;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_WML11;
        p->publicID = &sv_wml11_public_id;
        p->tagTable = sv_wml11_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wml11_attr_table;
        p->attrValueTable = sv_wml11_attr_value_table;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_WML12;
        p->publicID = &sv_wml12_public_id;
        p->tagTable = sv_wml12_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wml12_attr_table;
        p->attrValueTable = sv_wml12_attr_value_table;
        p->extValueTable = NULL;
        p++;
#else /* WBXML_TABLES_SEPARATE_WML_VERSIONS */
        p->langID = WBXML_LANG_WML10;
        p->publicID = &sv_wml10_public_id;
        p->tagTable = sv_wml13_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wml13_attr_table;
        p->attrValueTable = sv_wml13_attr_value_table;
        p->extValueTable = NULL;
        p++;
        ...
        p++;    
#endif /* WBXML_TABLES_SEPARATE_WML_VERSIONS */
        p->langID = WBXML_LANG_WML13;
        p->publicID = &sv_wml13_public_id;
        p->tagTable = sv_wml13_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wml13_attr_table;
        p->attrValueTable = sv_wml13_attr_value_table;
...
#if defined( WBXML_SUPPORT_WTA )
        p->langID = WBXML_LANG_WTA10;
        p->publicID = &sv_wta10_public_id;
        p->tagTable = sv_wta10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wta10_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_WTAWML12;
        p->publicID = &sv_wtawml12_public_id;
        p->tagTable = sv_wtawml12_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wtawml12_attr_table;
        p->attrValueTable = sv_wtawml12_attr_value_table;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_CHANNEL11;
        p->publicID = &sv_channel11_public_id;
        p->tagTable = sv_channel11_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_channel11_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_CHANNEL12;
        p->publicID = &sv_channel12_public_id;
        p->tagTable = sv_channel12_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_channel12_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;
#endif /* WBXML_SUPPORT_WTA */
...
#if defined( WBXML_SUPPORT_SI )
        p->langID = WBXML_LANG_SI10;
        p->publicID = &sv_si10_public_id;
        p->tagTable = sv_si10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_si10_attr_table;
        p->attrValueTable = sv_si10_attr_value_table;
        p->extValueTable = NULL;
        p++;
...
#if defined( WBXML_SUPPORT_SL )
        p->langID = WBXML_LANG_SL10;
        p->publicID = &sv_sl10_public_id;
        p->tagTable = sv_sl10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_sl10_attr_table;
        p->attrValueTable = sv_sl10_attr_value_table;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_SL */

#if defined( WBXML_SUPPORT_CO )
        p->langID = WBXML_LANG_CO10;
        p->publicID = &sv_co10_public_id;
        p->tagTable = sv_co10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_co10_attr_table;
        p->attrValueTable = sv_co10_attr_value_table;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_CO */

#if defined( WBXML_SUPPORT_PROV )
        p->langID = WBXML_LANG_PROV10;
        p->publicID = &sv_prov10_public_id;
        p->tagTable = sv_prov10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_prov10_attr_table;
        p->attrValueTable = sv_prov10_attr_value_table;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_PROV */

#if defined( WBXML_SUPPORT_EMN )
        p->langID = WBXML_LANG_EMN10;
        p->publicID = &sv_emn10_public_id;
        p->tagTable = sv_emn10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_emn10_attr_table;
        p->attrValueTable = sv_emn10_attr_value_table;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_EMN */

#if defined( WBXML_SUPPORT_DRMREL )
        p->langID = WBXML_LANG_DRMREL10;
        p->publicID = &sv_drmrel10_public_id;
        p->tagTable = sv_drmrel10_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_drmrel10_attr_table;
        p->attrValueTable = sv_drmrel10_attr_value_table;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_DRMREL */

#if defined( WBXML_SUPPORT_OTA_SETTINGS )
        p->langID = WBXML_LANG_OTA_SETTINGS;
        p->publicID = &sv_ota_settings_public_id;
        p->tagTable = sv_ota_settings_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_ota_settings_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;    
#endif /* WBXML_SUPPORT_OTA_SETTINGS */
...
#if defined( WBXML_SUPPORT_SYNCML )
        /* SyncML 1.2 */
        p->langID = WBXML_LANG_SYNCML_SYNCML12;
        p->publicID = &sv_syncml_syncml12_public_id;
        p->tagTable = sv_syncml_syncml12_tag_table;
        p->nsTable = sv_syncml_syncml12_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_SYNCML_DEVINF12;
        p->publicID = &sv_syncml_devinf12_public_id;
        p->tagTable = sv_syncml_devinf12_tag_table;
        p->nsTable = sv_syncml_devinf12_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_SYNCML_METINF12;
        p->publicID = &sv_syncml_metinf12_public_id;
        p->tagTable = sv_syncml_metinf12_tag_table;
        p->nsTable = NULL;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        ...
        p->langID = WBXML_LANG_SYNCML_DMDDF12;
        p->publicID = &sv_syncml_dmddf12_public_id;
        p->tagTable = sv_syncml_dmddf12_tag_table;
        p->nsTable = sv_syncml_dmddf12_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;
        ...
        /* SyncML 1.1 */
        p->langID = WBXML_LANG_SYNCML_SYNCML11;
        p->publicID = &sv_syncml_syncml11_public_id;
        p->tagTable = sv_syncml_syncml11_tag_table;
        p->nsTable = sv_syncml_syncml11_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_SYNCML_DEVINF11;
        p->publicID = &sv_syncml_devinf11_public_id;
        p->tagTable = sv_syncml_devinf11_tag_table;
        p->nsTable = sv_syncml_devinf11_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_SYNCML_METINF11;
        p->publicID = &sv_syncml_metinf11_public_id;
        p->tagTable = sv_syncml_metinf11_tag_table;
        p->nsTable = NULL;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        ...
        /** @todo Check if Tag Tables are exactly with SyncML 1.0 */
        p->langID = WBXML_LANG_SYNCML_SYNCML10;
        p->publicID = &sv_syncml_syncml10_public_id;
        p->tagTable = sv_syncml_syncml11_tag_table;
        p->nsTable = sv_syncml_syncml10_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_SYNCML_DEVINF10;
        p->publicID = &sv_syncml_devinf10_public_id;
        p->tagTable = sv_syncml_devinf11_tag_table;
        p->nsTable = sv_syncml_devinf11_ns_table;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
...
#if defined( WBXML_SUPPORT_WV )
        p->langID = WBXML_LANG_WV_CSP11;
        p->publicID = &sv_wv_csp11_public_id;
        p->tagTable = sv_wv_csp_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wv_csp_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = sv_wv_csp_ext_table;
        p++;

        p->langID = WBXML_LANG_WV_CSP12;
        p->publicID = &sv_wv_csp12_public_id;
        p->tagTable = sv_wv_csp_tag_table;
        p->nsTable = NULL;
        p->attrTable = sv_wv_csp_attr_table;
        p->attrValueTable = NULL;
...
#if defined( WBXML_SUPPORT_AIRSYNC )
        p->langID = WBXML_LANG_AIRSYNC;
        p->publicID = &sv_airsync_public_id;
        p->tagTable = sv_airsync_tag_table;
        p->nsTable = sv_airsync_ns_table;
        p->attrTable = sv_airsync_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;

        p->langID = WBXML_LANG_ACTIVESYNC;
        p->publicID = &sv_activesync_public_id;
        p->tagTable = sv_airsync_tag_table;
        p->nsTable = sv_activesync_ns_table;
        p->attrTable = sv_airsync_attr_table;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
        p++;    
...
#if defined( WBXML_SUPPORT_CONML )
        p->langID = WBXML_LANG_CONML;
        p->publicID = &sv_conml_public_id;
        p->tagTable = sv_conml_tag_table;
        p->nsTable = NULL;
        p->attrTable = NULL;
        p->attrValueTable = NULL;
        p->extValueTable = NULL;
                                                                ...
                                                                const WB_UTINY *root)
{
    ...
    if (root != NULL) {
        ...
        sep = (WB_UTINY *)WBXML_STRRCHR((const WB_TINY *) root, WBXML_NAMESPACE_SEPARATOR);
        if (sep != NULL) {
...
WBXML_DECLARE(WB_UTINY) wbxml_tables_get_code_page(const WBXMLNameSpaceEntry *ns_table, const WB_TINY* xmlns)
{
    ...
    while (ns_table[i].xmlNameSpace != NULL)
    {
        if (WBXML_STRCMP(ns_table[i].xmlNameSpace, xmlns) == 0)
