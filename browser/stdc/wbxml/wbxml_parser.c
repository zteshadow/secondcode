...
static WB_BOOL check_public_id(WBXMLParser *parser)
{
    ...
    if (parser->public_id_index != -1) {
        ...
        while (parser->mainTable[index].publicID != NULL)
        {
            if ((parser->mainTable[index].publicID->xmlPublicID[0] != 0) && 
...
static WBXMLError parse_tag(WBXMLParser *parser, WB_UTINY *tag, WBXMLTag **element)
{
    ...
    while ((parser->langTable->tagTable[index].xmlName[0] != 0) &&
    ...
    if (parser->langTable->tagTable[index].xmlName[0] == 0) {
...
static WBXMLError parse_extension(WBXMLParser *parser, WBXMLTokenType code_space, WBXMLBuffer **result)
{
    ...
    case WBXML_LANG_WTAWML12:
        ...
        /* Generate "$(" */
        cm_memcpy(ext + len, var_begin, WBXML_STRLEN(var_begin));
        len += WBXML_STRLEN(var_begin);
    
        /* Generate 'variable' */
        cm_memcpy(ext + len, wbxml_buffer_get_cstr(var_value), wbxml_buffer_len(var_value));
        ...
        case WBXML_EXT_T_0:
            /* Generate ":escape" */
            cm_memcpy(ext + len, escape, WBXML_STRLEN(escape));
        ...
        case WBXML_EXT_T_1:
            /* Generate ":unesc" */
            cm_memcpy(ext + len, unesc, WBXML_STRLEN(unesc));
        ...
        case WBXML_EXT_T_2:
            /* Generate ":noesc" */
            cm_memcpy(ext + len, noesc, WBXML_STRLEN(noesc));
        ...
        /* Generate ")" */
        cm_memcpy(ext + len, var_end, WBXML_STRLEN(var_end));
    ...
    case WBXML_LANG_WV_CSP12:
        ...
        while ((parser->langTable->extValueTable[tab_index].xmlName[0] != 0) &&
               (parser->langTable->extValueTable[tab_index].wbxmlToken != ext_value) )
        {
            tab_index++;
        }
    
        if (parser->langTable->extValueTable[tab_index].xmlName[0] == 0) {
#if WBXML_PARSER_BEST_EFFORT
                                   ...
                                   const WB_UTINY     **value)
{
    ...
    while ((parser->langTable->attrTable[index].xmlName[0] != 0) &&
    ...
    if (parser->langTable->attrTable[index].xmlName[0] == 0) {
#if WBXML_PARSER_BEST_EFFORT
    ...
    if (parser->langTable->attrTable[index].xmlValue[0] != 0) {
                                   ...
                                   WBXMLBuffer **result)
{
    ...
    while ((parser->langTable->attrValueTable[index].xmlName[0] != 0) &&
    ...
    if (parser->langTable->attrValueTable[index].xmlName[0] == 0) {
...
static WBXMLError decode_wv_integer(WBXMLBuffer **data)
{
    ...
    WBXML_SPRINTF((WB_TINY *)tmp, "%u", the_int);
